...
class YIFF {
    ...
    async sendRequest(cat, method, amount) {
        ...
        if (amount && amount > 5)
            throw new TypeError("Ammount cannot be greater than five.");
        const h = {};
        ...
        switch (method) {
            case "image": {
                ...
                const r = await get_1.default(`${this.options.baseURL}/${Constants_1.API_VERSION}/furry/yiff/${cat}/image`, this.options.userAgent, this.options.apiKey);
                ...
                this.debug(`${this.options.baseURL}/${Constants_1.API_VERSION}/furry/yiff/${cat}/image`, { start, end, time: parseFloat((end - start).toFixed(2)) });
                return {
                    image: r.body,
                    data: d
                };
            ...
            case "json": {
                const start = perf_hooks_1.performance.now();
                const r = await get_1.default(`${this.options.baseURL}/${Constants_1.API_VERSION}/furry/yiff/${cat}`, this.options.userAgent, this.options.apiKey);
                const end = perf_hooks_1.performance.now();
                if (r.statusCode !== 200) {
                    const e = ErrorHandler_1.default(r.statusCode);
                    if (!e)
                ...
                this.debug(`${this.options.baseURL}/${Constants_1.API_VERSION}/furry/yiff/${cat}`, { start, end, time: parseFloat((end - start).toFixed(2)) });
                return amount === 1 ? b.images[0] : b.images;
                break;
            }
        }
    }
    get dickgirl() { return this.sendRequest.bind(this, "dickgirl"); }
