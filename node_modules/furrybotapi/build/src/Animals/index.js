    ...
    async sendRequest(cat, method, amount) {
        ...
        switch (method) {
            case "image": {
                ...
                const r = await get_1.default(`${this.options.baseURL}/${Constants_1.API_VERSION}/animals/${cat}/image`, this.options.userAgent, this.options.apiKey);
                ...
                this.debug(`${this.options.baseURL}/${Constants_1.API_VERSION}/animals/${cat}/image`, { start, end, time: parseFloat((end - start).toFixed(2)) });
                return {
                    image: r.body,
                    data: d
                };
            ...
            case "json": {
                const start = perf_hooks_1.performance.now();
                const r = await get_1.default(`${this.options.baseURL}/${Constants_1.API_VERSION}/animals/${cat}`, this.options.userAgent, this.options.apiKey);
                const end = perf_hooks_1.performance.now();
                if (r.statusCode !== 200) {
                    const e = ErrorHandler_1.default(r.statusCode);
                    if (!e)
                ...
                this.debug(`${this.options.baseURL}/${Constants_1.API_VERSION}/animals/${cat}`, { start, end, time: parseFloat((end - start).toFixed(2)) });
